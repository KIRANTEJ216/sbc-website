          ...
          <GsapReveal>
            <div className="py-4 sm:py-8">
              <div className="relative">
                <div className="h-[3px] bg-gradient-to-r from-accent/30 via-accent/60 to-accent absolute left-0 right-0 top-1/2 -translate-y-1/2 rounded-full" />
                <div className="flex items-center justify-between relative overflow-visible pb-8 sm:pb-6">
                  {Array.from({ length: totalMeetings }, (_, i) => (
                    <div key={i} className="flex flex-col items-center relative">
                      <div
                        className={`w-3 sm:w-4 h-3 sm:h-4 rotate-45 z-10 transition-all duration-300 shrink-0 ${
                          i < 5
                            ? "bg-accent/30"
                            : i < 10
                            ? "bg-accent/60"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {stats.map((s, i) => (
              <GsapReveal key={s.label} variant="zoom" delay={0.1 * i}>
                <div className="glass rounded-[1.75rem] p-6 sm:p-8 text-center border border-border/60 stat-accent-line transition-all duration-500 hover:shadow-card-glow hover:-translate-y-1">
                  <p className="text-2xl sm:text-3xl font-heading font-bold gradient-accent">
